...
ALTER TABLE lot ADD COLUMN iduserinsert INTEGER NOT NULL DEFAULT @idUser;
ALTER TABLE lot ADD COLUMN datemodif DATETIME NOT NULL DEFAULT NOW();
ALTER TABLE lot ADD COLUMN idusermodif INTEGER NOT NULL DEFAULT @idUser;
create table dirvente
(
    idUser                    INTEGER   UNSIGNED    not null,
    clesession VARCHAR(256) NULL,
    constraint PK_DIRVENTE primary key (idUser),
  	constraint FK_DIRVENTE_UTILISATEUR foreign key  (idUser) references utilisateur (id)
    
) ENGINE = INNODB;
INSERT INTO dirvente(idUser) VALUES (SELECT id FROM utilisateur WHERE idtypeuser=4);
DROP PROCEDURE Auth;
DELIMITER $
